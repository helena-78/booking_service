<template>
  <div class="user-suggestions">
    ...
    <div v-else-if="suggestion && suggestion.suggestedUsers.length === 0" class="status">
      <p>No suggestions found for the selected filters.</p>
    </div>

    <div v-else-if="suggestion" class="suggestions-list">
      <p class="count">
        Found <strong>{{ suggestion.suggestedUsers.length }}</strong> suggestion(s)
        <span class="meta"> — Computed: {{ formatDate(suggestion.createdAt) }}</span>
      </p>

      <div
        v-for="(user, index) in suggestion.suggestedUsers"
        :key="user.userId"
        class="suggestion-card"
      >
        <div class="suggestion-header">
          <span class="suggestion-rank">#{{ index + 1 }}</span>
          <span class="suggestion-id">User ID: {{ user.userId }}</span>
          <span class="suggestion-score" :class="scoreClass(user.compatibilityScore)">
            {{ pct(user.compatibilityScore) }} match
          </span>
        </div>

        <div class="suggestion-row">
          <strong>Sport:</strong> {{ pct(user.sportScore) }}
        </div>
        <div class="suggestion-row">
          <strong>Skill:</strong> {{ pct(user.skillScore) }}
        </div>
        <div class="suggestion-row">
          <strong>Location:</strong> {{ pct(user.locationScore) }}
        </div>
        <div class="suggestion-row">
          <strong>Rating:</strong> {{ pct(user.ratingScore) }}
        </div>
        <div class="suggestion-row">
          <strong>Language:</strong> {{ pct(user.languageScore) }}
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    call(method) {
      ...
      const url = `${BASE}/users/${this.userId.trim()}/suggestions?${params}`
